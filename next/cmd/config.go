...
func (c *Config) applyArgs(targetSystem chezmoi.System, targetDir string, args []string, include *chezmoi.IncludeSet, recursive bool, umask os.FileMode) error {
	...
	for _, targetName := range targetNames {
		if err := s.ApplyOne(targetSystem, c.persistentState, targetDir, targetName, applyOptions); err != nil {
			if c.keepGoing {
				c.errorf("%v", err)
			} else {
				return err
...
func (c *Config) destPath(arg *chezmoi.OSPath) (string, error) {
	path, err := arg.TildeAbsSlash(c.HomeDirStr)
	if err != nil {
		return "", err
	}
...
func (c *Config) persistentPreRunRootE(cmd *cobra.Command, args []string) error {
	...
	var err error
	c.absSlashConfigFile, err = chezmoi.NewOSPath(c.configFileStr).TildeAbsSlash(c.HomeDirStr)
	if err != nil {
		return err
	}
	...
	c.absSlashSourceDir, err = chezmoi.NewOSPath(c.SourceDirStr).TildeAbsSlash(c.HomeDirStr)
	if err != nil {
		return err
	}
	c.absSlashDestDir, err = chezmoi.NewOSPath(c.DestDirStr).TildeAbsSlash(c.HomeDirStr)
	if err != nil {
		return err
	}

	switch cmd.Annotations[persistentStateMode] {
	case persistentStateModeEmpty:
		c.persistentState = chezmoi.EmptyPersistentState{}
	case persistentStateModeReadOnly:
	...
	case persistentStateModeReadWrite:
		persistentStateFile := c.persistentStateFile().String()
		if c.dryRun {
